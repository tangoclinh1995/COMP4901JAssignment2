...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        outHidden, cacheHidden = affine_relu_forward(
            X, self.params["W1"], self.params["b1"]
        )
        
        scores, cacheScore = affine_forward(
            outHidden, self.params["W2"], self.params["b2"]
        )       
        ...
        # Gradient at Softmax step. Also loss
        loss, dout = softmax_loss(scores, y)
        loss += 0.5 * self.reg * np.sum(W1 * W1) + 0.5 * self.reg * np.sum(W2 * W2)
        
        # Gradient at Hidden layer
        dOutHidden, grads["W2"], grads["b2"] = affine_backward(dout, cacheScore)
        grads["W2"] += self.reg * W2
        
        # Gradient at Input layer
        _, grads["W1"], grads["b1"] = affine_relu_backward(dOutHidden, cacheHidden)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        WLayer = "W" + str(self.num_layers)
        bLayer = "b" + str(self.num_layers)
        
        dx, grads[WLayer], grads[bLayer] = affine_backward(
            dSoft,
            cacheScore
        )
