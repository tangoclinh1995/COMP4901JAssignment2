...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    # Transform x to fit the function batchnorm_forward
    #   Order (N, C, H, W) -> Order (N, H, W, C) -> Shape (N * H * W, C)
    N, C, H, W = x.shape
    
    xTransformed = x.transpose((0, 2, 3, 1, )).reshape((N * H * W, C, ))
    
    outTransformed, cache = batchnorm_forward(
        xTransformed, gamma, beta, bn_param
    )
    
    # Transform outTransform to fit with original x
    out = outTransformed.reshape((N, H, W, C, )).transpose((0, 3, 1, 2, ))
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dOutTransformed = dout.transpose((0, 2, 3, 1, )).reshape((N * H * W, C, ))
    
    dXTransformed, dgamma, dbeta = batchnorm_backward(dOutTransformed, cache)

    dx = dXTransformed.reshape((N, H, W, C, )).transpose((0, 3, 1, 2, ))    
